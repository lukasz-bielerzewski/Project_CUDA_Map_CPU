...
namespace walkers{
    ...
    Quaternion toQuaternion(const Vec3& rpy){
        return Quaternion(toRotationMat(rpy));
    }

    /// compute rotation matrix from axis and angle
    Mat33 fromAxisAndAngle(const Vec3& axis, double theta){
        Mat33 mat(Mat33::Identity());
        mat(0,0) = cos(theta)+axis.x()*axis.x()*(1-cos(theta));
        mat(0,1) = axis.x()*axis.y()*(1-cos(theta))-axis.z()*sin(theta);
        mat(0,2) = axis.x()*axis.z()*(1-cos(theta))+axis.y()*sin(theta);

        mat(1,0) = axis.y()*axis.x()*(1-cos(theta))+axis.z()*sin(theta);
        mat(1,1) = cos(theta)+axis.y()*axis.y()*(1-cos(theta));
        mat(1,2) = axis.y()*axis.z()*(1-cos(theta))-axis.x()*sin(theta);

        mat(2,0) = axis.z()*axis.x()*(1-cos(theta))-axis.y()*sin(theta);
        mat(2,1) = axis.z()*axis.y()*(1-cos(theta))+axis.x()*sin(theta);
        mat(2,2) = cos(theta)+axis.z()*axis.z()*(1-cos(theta));
        return mat;
    }
    ...
    Mat33 fromAxis(const Vec3& _normal, const std::string& axisName){
        ...
        if (axisName=="z"){
            ...
            x.vector().normalize();
            R.matrix().block(0,0,3,1) = x.vector();
            R.matrix().block(0,1,3,1) = y.vector();
            R.matrix().block(0,2,3,1) = normal.vector();
        }
        return R;
    }
    ...
    double angleBetweenVectors(const Vec3& vec1, const Vec3& vec2){
        return acos(vec1.vector().dot(vec2.vector()));
    }
